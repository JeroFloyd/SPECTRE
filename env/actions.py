...
PRIMITIVES = ["parse_data", "validate_data", "transform_data", "export_result"]

ACTION_TYPES = ["primitive", "create_tool", "use_tool"]


def validate_action(action: dict, known_tools: list[str]) -> str | None:
    if not isinstance(action, dict):
        return "Action must be a JSON object (dict)"
    ...
    if atype == "primitive":
        ...
        if name not in known_tools:
            return f"Tool '{name}' not defined. Available tools: {known_tools}"
